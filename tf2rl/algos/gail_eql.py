...
from tf2rl.networks.layer import DenseLayer, DenseLayerv2, EqlLayerv2 as EqlLayer
...
class Discriminator_EQL(tf.keras.Model):
    def __init__(self, state_shape, action_dim, num_layers=2, drop_out = None,
        ...
        self.layer_out = DenseLayerv2(w_initializer=self.w_initializer, b_initializer=self.b_initializer,
                                    constraint=constraint, lmbda=lmbda, dynamic_lmbda=self._is_lmbda_dynamic,
...
class GAIL_EQL(IRLPolicy):
    ...
    def _init(self):
        self.disc = Discriminator_EQL(state_shape=self.state_shape, action_dim=self.action_dim, v=self.v, drop_out=self.drop_out,
        ...
        if self.atol_starting_step or self.tau > 0:
            self.disc_target = Discriminator_EQL(state_shape=self.state_shape, action_dim=self.action_dim, v=self.v,drop_out=self.drop_out,
                                          num_layers=self.num_layers, enable_sn=self.enable_sn, constraint=None,
        ...
        # self.optimizer = tf.keras.optimizers.RMSprop(learning_rate=lr) # for WGAN

    def train(self, agent_states, agent_acts, expert_states, expert_acts, itr, **kwargs):
        assert itr , "The interation must be defined [train GAIL_EQL]"
        # self._train_body_gp is moved to dac
        train_fun = self._train_body # if self.grad_penalty_coeff is None else self._train_body_gp
        ...
        if itr % self.update_delay == 0:
            ...
            loss, accuracy, js_divergence, grads = train_fun(agent_states, agent_acts, expert_states, expert_acts, l1_regularizers)
    ...
    #TODO: Need to investigate to see if it is better for this function to be inside the _train_body
    # or to be decorated with tf.function
    @tf.function
    def _compute_js_divergence(self, fake_logits, real_logits):
        print("[DEBUG] initializing [_compute_js_divergence GAIL_EQL] shape: {TODO}")
    ...
    def _train_body(self, agent_states, agent_acts, expert_states, expert_acts, l1_regularizers):
        print("[DEBUG] initializing [_train_body GAIL_EQL] shape: {TODO}")
        epsilon = 1e-8
        with tf.device(self.device):
            with tf.GradientTape(watch_accessed_variables=False) as tape: # watch_accessed_variables=False
                ...
                real_logits = self.disc(tf.concat((expert_states, expert_acts), axis=1), l1_regularizers=l1_regularizers)
                fake_logits = self.disc(tf.concat((agent_states, agent_acts), axis=1), l1_regularizers=l1_regularizers)
                ...
                if self._is_grad_penalty:
                    ...
                    loss = -(tf.reduce_mean(tf.math.log(real_logits + epsilon)) +
                             ...
                             1 * tf.reduce_sum(self.disc.losses))
                else:
                    # GAN like loss
                    loss = -(tf.reduce_mean(tf.math.log(real_logits + epsilon)) +
                             tf.reduce_mean(tf.math.log(1. - fake_logits + epsilon)) -
    ...
    def inference(self, states, actions, next_states=None, apply_atol=False):
    ...
    def _inference_body_atol(self, inputs):
        print("[DEBUG] initializing [_inference_body_atol GAIL_EQL] shape: {} name: {}-{}".format(inputs.shape,
                                                                                             self.disc.name,
    ...
    def _inference_body(self, inputs):
        print("[DEBUG] initializing [_inference_body GAIL_EQL] shape: {}".format(inputs.shape))
        with tf.device(self.device):
            if self.tau > 0:
