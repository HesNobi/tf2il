...
class SAC_IRL(OffPolicyAgent):
    ...
    def train(self, states, actions, next_states, rewards, dones, weights=None):
        ...
        if self._is_absorbed_state:
            train_body = self._train_body_absorbed
        else:
            train_body = self._train_body
    ...
    @tf.function
    def _train_body_absorbed(self, states, actions, next_states, rewards, dones, weights):
        ...
        with tf.device(self.device):
            ...
            # not_dones = 1. - tf.cast(dones, dtype=tf.float32)
            not_dones = tf.maximum(tf.cast(dones, dtype=tf.float32), 0.)
            q_mask = tf.broadcast_to(tf.transpose(tf.expand_dims(not_dones, axis=0)),
                                     [not_dones.shape[0], actions.shape[-1]])
    ...
    def _compute_td_error_body(self, states, actions, next_states, rewards, dones):
        print("[DEBUG] initializing {_compute_td_error_body SAC_IRL}...")
        with tf.device(self.device):
            not_dones = 1. - tf.cast(dones, dtype=tf.float32)
