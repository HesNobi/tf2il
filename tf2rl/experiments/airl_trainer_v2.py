...
class AIRLTrainerV2(IRLTrainer):
    def __init__(
            self,
            policy,
            env,
        ...
        assert hasattr(self._env, "_max_episode_steps"), "[ERROR]The AIRLTrainerV2 need env._max_episode_steps set."
        assert isinstance(self._policy, SAC), "[ERROR] Only works with stoastic policies"
    ...
    def __call__(self):
        ...
        while total_steps < self._max_steps:
            ...
            if done or episode_steps == self._env._max_episode_steps:
                if self._is_episodic and done and episode_steps < self._env._max_episode_steps:
                    ...
                    replay_buffer.add(obs=obs_abs, act=act_abs,
                                      ...
                                      done=Done.ABSORBING.value, logp=np.array(0.))
            ...
            if total_steps % self._policy.update_interval == 0:
                ...
                if "EQL" in self._irl.policy_name and self._irl.atol_starting_step:
                    raise NotImplemented("[ERROR] No ATOL for AIRL!")
                    ...
                    if not self._is_airl_sac:
                        ...
                        raise NotImplemented("[ERROR] AIRL_SAC generator is not ready.")
